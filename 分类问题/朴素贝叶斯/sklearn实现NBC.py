...
def spamTest():
    ...
    for i in testSet:
        ...
        testLabel.append(classList[i])
        testMat.append(wordVector)
    data_predicted = model.predict(array(testMat))
    mess = metrics.classification_report(array(testLabel), data_predicted)
    # 输出分类评价
    print(mess)
    labelNos = list(set(classList))
    mess2 = metrics.confusion_matrix(testLabel, data_predicted, labelNos)
    # 输出混淆矩阵
    print(mess2)
